        ...
        <table class="table table-middle table-last-td-align-right">
            ...
            {foreach from=$orders.0 item="order"}
                <tr>
                    ...
                    <td><a href="{"orders.details?order_id=`$order.order_id`"|fn_url}"><bdi>{__("order")}</bdi> #{$order.order_id}</a> {__("by")} {if $order.user_id}<a href="{"profiles.update?user_id=`$order.user_id`"|fn_url}">{/if}{$order.lastname} {$order.firstname}{if $order.user_id}</a>{/if}</td>
                        ...
                        <div class="scrollable-table">
                            <table class="table table-striped">
                                ...
                                {foreach $top_sellers as $seller}
                                    <tr>
                                        <td width="34%">
                                            <a href="{"orders.manage?is_search=Y&period=C&time_from=`$time_from`&time_to=`$time_to`&company_id=`$seller.company_id`"|fn_url}"
                                            >{$seller.company}</a>
                                        </td>
                                        <td width="33%" class="right">{include file="common/price.tpl" value=$seller.total_sales}</td>
                                        {if $addons.vendor_plans.status == "A"}
                                            <td width="33%" class="right">{include file="common/price.tpl" value=$seller.total_commission}</td>
                                        {/if}
                                    </tr>
                                {/foreach}
                                </tbody>
                    ...
                    {if $item}
                        <div class="item">
                            {hook name="index:recent_activity"}
                                ...
                                {__($_type)}{if $item.action}&nbsp;<bdi>({__($_action)})</bdi>{/if}:

                                {if $item.type == "users" && "profiles.update?user_id=`$item.content.id`"|fn_url|fn_check_view_permissions:"GET"}
                                    {if $item.content.id}<a href="{"profiles.update?user_id=`$item.content.id`"|fn_url}">{/if}<bdi>{$item.content.user}{if $item.content.id}</bdi></a>{/if}<br>
                                ...
                                {elseif $item.type == "orders" && "orders.details?order_id=`$item.content.id`"|fn_url|fn_check_view_permissions:"GET"}
                                    ...
                                    <a href="{"orders.details?order_id=`$item.content.id`"|fn_url}"><bdi>{__("order")}&nbsp;{$item.content.order}</bdi></a><br>
                                {elseif $item.type == "products" && "products.update?product_id=`$item.content.id`"|fn_url|fn_check_view_permissions:"GET"}
                                    <a href="{"products.update?product_id=`$item.content.id`"|fn_url}"><bdi>{$item.content.product}</bdi></a><br>

                                {elseif $item.type == "categories" && "categories.update?category_id=`$item.content.id`"|fn_url|fn_check_view_permissions:"GET"}
                                    <a href="{"categories.update?category_id=`$item.content.id`"|fn_url}"><bdi>{$item.content.category}</bdi></a><br>
                                {/if}
