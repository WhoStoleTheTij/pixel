...
class Settings
{
    ...
    public static function getValueReadable($setting_data, $value = null)
    {
        ...
        switch ($setting_data['type']) {
            case 'P': // password
                return str_repeat('*', 8);
            case 'C': // checkbox
                return ($value == 'Y' || $value === true) ? __("yes"): __("no");
            case 'R': // radiogroup
            case 'S': // selectbox
            case 'K': // selectbox with variants from multiple checkboxes (type = G)
                return isset($setting_data['variants'][$value]) ? $setting_data['variants'][$value] : __('none');
            case 'M': // multiple select
            case 'N': // multiple checkboxes
            case 'G': // multiple checkboxes for selectbox (type = K)
            case 'B': // selectable box
                $values = array();
            ...
            case 'X': // country
                ...
                return $value;
            case 'E': // add-on template
            case 'Z': // add-on template
            case 'H': // header
                return '';
            case 'O': // result of a callable handler
                return $setting_data['info'];
            case 'W': // states: we are unable to determine the state without a country
            case 'F': // file
            case 'T': // textarea
            case 'I': // text
            case 'U': // numeric
     ...
     *
     * @param bool $use_access_level Whether to perform check as a company administator
     *
    ...
    public function getEditableSettingsEditionTypes($use_access_level = true)
    {
        ...
        foreach ($edition_types as $type) {
            $edition_types[] = $this->_getCurrentEdition() . $type;
        }
