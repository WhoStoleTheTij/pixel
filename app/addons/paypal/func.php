    ...
    if (empty($order_info['use_gift_certificates']) && !floatval($order_info['subtotal_discount']) && empty($order_info['points_info']['in_use']) && count($order_info['products']) < $max_pp_products) {
        ...
        if (!empty($order_info['taxes']) && Registry::get('settings.General.tax_calculation') == 'subtotal') {
            foreach ($order_info['taxes'] as $tax_id => $tax) {
                ...
                $suffix = '_' . ($i++);
                $item_name = strip_tags($tax['description']);
...
function fn_validate_paypal_signup_request($request = array())
{
    if (isset($request['nonce'])) {
        ...
        unset($request['dispatch'], $request['nonce']);
        ksort($request);
...
)
{
    ...
    $request_data = array(
        ...
        'store_url' => $store_url ?: fn_url("paypal_connector.end_signup?payment_id={$payment_id}{$company_suffix}", 'C', $protocol),
